insert into authors (name)
values ('Jon Daw')
ON CONFLICT (name) DO NOTHING;
;

insert into authors (name)
values ('Егор Бугаенко')
ON CONFLICT (name) DO NOTHING;
;

insert into book(title, isbn, edition, publish_year)
values ('simple book', '111111', '1', '2021')
on conflict do nothing;


insert into book_author_t (book_id, author_id)
select b.id, a.id
from book b
         join authors a
              on true
where a.name in ('Jon Daw', 'Егор Бугаенко')
  and b.title = 'simple book'
  and b.edition = 1

  and not exists(
        select null
        from book_author_t tt
        where tt.book_id = b.id
    )

on conflict do nothing;


insert into genre (name)
select 'computer science' as n
union
select 'technics'
union
select  'fiction'
on conflict (name) do nothing ;

insert into  users (name , password , enabled)
select 'admin' , '$2y$12$NeTmvzlXLBe.Dgeyo6Mqseh4Ev9UJ/I9gNu1hSeGywqoMDlq5JAMW', true union all
select 'user' , '$2y$12$NeTmvzlXLBe.Dgeyo6Mqseh4Ev9UJ/I9gNu1hSeGywqoMDlq5JAMW', true
on conflict (name) do nothing ;


insert into roles (name)
select 'ADMIN' union all
select 'USER' on conflict
    do nothing ;


insert into users_roles (user_id, roles_id)

select u.id, r.id from users u join roles r on true where u.name = 'admin'
                                                      and  r.name = 'ADMIN'
union all
select u.id, r.id from users u join roles r on true where u.name = 'user'
                                                      and  r.name = 'USER'
on conflict  do nothing;

